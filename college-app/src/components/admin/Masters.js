...
const MastersAdminTemplate = () => {
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {formData.applicationRequirements.map((req, i) => (
            <div key={i} style={S.reqTag}>
              <span>{req}</span>
              <button style={S.reqTagBtn} onClick={() => handleRequirementRemove(i)}><IconX size={11}/></button>
            </div>
          ))}
        </div>
      </div>
  ...
  const renderPreview = () => (
    <div style={S.overlay} onClick={() => setShowPreview(false)}>
      <div style={S.previewSheet} onClick={e => e.stopPropagation()}>
        {/* Handle */}
        <div style={S.previewHandle}>
          <div style={S.previewTitle}>Preview</div>
          <button style={S.previewCloseBtn} onClick={() => setShowPreview(false)}><IconX size={13}/></button>
        </div>
        ...
        <div style={S.previewFooter}>
          <button style={{ ...S.btnPrev, flex: 1 }} onClick={() => { setShowPreview(false); setCurrentStep(1); }}>
  ...
  return (
    <>
      ...
      <div style={S.container}>
        ...
        <div style={S.header}>
          <div style={S.headerTitle}>
            ...
            {editingUniversity ? "Edit University" : "Add University — Master's"}
          </div>
          <div style={S.headerSub}>Create and manage Master's program profiles</div>
        </div>
        ...
        {apiError && (
          <div style={S.errBanner}>
            ...
            <button style={{ background: "none", border: "none", cursor: "pointer", color: "#991b1b", display: "flex" }} onClick={() => setApiError(null)}><IconX size={13}/></button>
          </div>
        )}
      ...
      {/* Fixed Bottom Nav */}
      <div style={{ ...S.navBar, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 520 }}>
        {currentStep > 1 ? (
          <button style={S.btnPrev} onClick={() => setCurrentStep(s => s - 1)}>
            <IconArrowLeft size={13}/> Back
          </button>
        ) : <div style={{ flex: 1 }}/>}

        {currentStep < 5 && (
          <button style={S.btnNext} onClick={() => setCurrentStep(s => s + 1)}>
            Next <IconArrowRight size={13}/>
          </button>
        )}

        {currentStep === 5 && (
          <button style={{ ...S.btnSubmit, opacity: loading ? 0.65 : 1 }} onClick={handleSubmit} disabled={loading}>
