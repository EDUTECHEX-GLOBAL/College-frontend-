...
const ProcessAdminLogin = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/process-admin/register",
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/process-admin/verify-otp",
        { email, otp: otpValue }
      );
      if (response.data.success) {
  ...
  const handleResendOtp = async () => {
    ...
    setIsLoading(true);
    try {
      await axios.post("http://localhost:5000/api/process-admin/resend-otp", { email });
      setResendCooldown(60);
      setOtp(["", "", "", "", "", ""]);
