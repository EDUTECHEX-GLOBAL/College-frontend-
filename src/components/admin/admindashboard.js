...
const lazyWithRetry = (factory, retries = 3) => {
  return lazy(async () => {
    let lastError;
...
const AdminUserManagement = lazyWithRetry(() => import("./adminuser"));
const Notifications = lazyWithRetry(() => import("./Notifications"));
const University = lazyWithRetry(() => import("./University"));
const Bachelors = lazyWithRetry(() => import("./Bachelors"));
const Masters = lazyWithRetry(() => import("./Masters"));
const StudentAnalytics = lazyWithRetry(() => import("./StudentAnalytics"));
const AdminVerifyDocuments = lazyWithRetry(() => import('./AdminVerifyDocuments'));
...
class LazySectionErrorBoundary extends React.Component {
  ...
  render() {
    ...
    return (
      <div className="lazy-section-error">
        <p>Failed to load this section.</p>
        <div className="lazy-section-error-actions">
...
const LazySection = ({ children, fallback = <TabFallback />, resetKey }) => (
  <LazySectionErrorBoundary resetKey={resetKey}>
...
const UsersSection = memo(({ userStats, loading }) => (
...
const UniversitySection = memo(({ uniStats, loading }) => (
  <div className="dash-section">
...
const ProgramsSection = memo(({ uniStats, loading }) => (
  <div className="dash-section">
    <SectionLabel label="BACHELOR'S & MASTER'S PROGRAMS" />
    <div className="stats-grid stats-grid-4">
      <StatCard title="Bachelor's Programs" value={uniStats.bachelorsTotalPrograms?.toLocaleString()} accent="blue"   icon={ICONS.bachelors("#0891b2")} loading={loading} />
...
const StudentSection = memo(({ studentStats, loading }) => (
  <div className="dash-section">
...
const RegistrationChart = memo(({ trend }) => {
...
const ProfileFunnel = memo(({ funnel, loading }) => {
...
const DashboardContent = memo(({
  ...
  <div className="dashboard-content">
    ...
    <UsersSection userStats={userStats} loading={loading} />
    <UniversitySection uniStats={uniStats} loading={loading} />
    <ProgramsSection uniStats={uniStats} loading={loading} />
    <StudentSection studentStats={studentStats} loading={loading} />

    <div className="charts-grid">
      <div className="chart-card"><RegistrationChart trend={trend} /></div>
      <div className="chart-card"><ProfileFunnel funnel={funnel} loading={loading} /></div>
    </div>
    ...
    <div className="quick-access-section">
      ...
      <div className="quick-action-cards">
        {[
          ...
          <div className="quick-action-card" key={tab} onClick={() => onTabChange(tab)}>
...
const AdminDashboard = () => {
  ...
  const loadDashboardData = useCallback(async (silent = false) => {
    ...
    try {
      ...
      // in-flight request instead of cancelling one valid request.
      if (!dashboardRequest) {
        const headers = authHeaders();
        dashboardRequest = fetch(`${API_BASE_URL}/api/admin/dashboard/overview`, { headers })
          .then(async (dashRes) => {
      ...
      const merged = await dashboardRequest;
  ...
  // ── Destructure data ──────────────────────────────────────────────────
  const d            = dashData || {};
  const userStats    = d.userStats          || EMPTY_USER_STATS;
  const uniStats     = d.universityStats    || EMPTY_UNI_STATS;
  const studentStats = d.studentAnalytics   || EMPTY_STUDENT_STATS;
  const trend        = d.registrationTrend  || EMPTY_TREND;
  const funnel       = d.profileFunnel      || EMPTY_FUNNEL;
  ...
  const renderContent = () => {
    if (activeTab === "notifications") {
      return (
        <div className="notifications-page">
          ...
          <LazySection resetKey={activeTab}>
    ...
    switch (activeTab) {
      ...
      case "users":             return <LazySection resetKey={activeTab}><AdminUserManagement /></LazySection>;
      case "university":        return <LazySection resetKey={activeTab}><University /></LazySection>;
      case "bachelors":         return <LazySection resetKey={activeTab}><Bachelors /></LazySection>;
      case "masters":           return <LazySection resetKey={activeTab}><Masters /></LazySection>;
     
      case "student-analytics": return <LazySection resetKey={activeTab}><StudentAnalytics /></LazySection>;
      case 'verify-documents':  return <LazySection resetKey={activeTab}><AdminVerifyDocuments /></LazySection>;
  ...
  const NAV_TITLE = {
    ...
    notifications: "Notifications", "student-analytics": "Student Analytics", "verify-documents": "Verify Documents", settings: "Settings",
  };
    ...
    <div className="admin-dashboard">
      ...
      <div className={`sidebar ${sidebarOpen ? "open" : "closed"}`}>
        ...
        <ul className="sidebar-menu">
          ...
          <MenuItem tab="verify-documents"  iconKey="verifyDocuments" label="Verify Documents" />
      ...
      <div className="main-content">
        <nav className="navbar">
          ...
          <div className="navbar-right">
            <LazySection fallback={null} resetKey="navbar-notifications">
