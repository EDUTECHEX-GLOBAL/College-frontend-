...
const University = () => {
  ...
  const fetchImportStats = async () => {
    ...
    if (!token) return;
    try {
      const response = await axios.get(`${API_BASE_URL}/api/admin/import-stats`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const importUniversities = async () => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/api/admin/import-universities`, {}, { headers: { Authorization: `Bearer ${token}` } });
  ...
  axios.get(`${API_BASE_URL}/api/admin/universities/search`, { 
    params: { q: searchTerm }, 
    headers: { Authorization: `Bearer ${token}` } 
  }).catch(() => ({ data: { success: false, data: [] } })),
  
  axios.get(`${API_BASE_URL}/api/bachelors/universities`, { 
    params: { search: searchTerm, limit: 500 },  // ← increased
    headers: { Authorization: `Bearer ${token}` } 
  }).catch(() => ({ data: { success: false, data: [] } })),
  
  axios.get(`${API_BASE_URL}/api/masters/universities`, { 
    params: { search: searchTerm, limit: 500 },  // ← increased
    headers: { Authorization: `Bearer ${token}` } 
  }).catch(() => ({ data: { success: false, data: [] } })),
]);
  ...
  const fetchAllUniversities = async () => {
    ...
    try {
      ...
      const [adminResponse, bachelorsResponse, mastersResponse] = await Promise.all([
        axios.get(`${API_BASE_URL}/api/admin/universities`, {
  ...
  const totalPages = Math.max(1, Math.ceil(displayData.length / UNIVERSITIES_PER_PAGE));
  const pageStartIndex = (currentPage - 1) * UNIVERSITIES_PER_PAGE;
  const pageEndIndex = Math.min(pageStartIndex + UNIVERSITIES_PER_PAGE, displayData.length);
  const paginatedData = displayData.slice(pageStartIndex, pageEndIndex);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, sortBy, filterType, programLevel, activeTab]);
        ...
        <>
          ...
          <div className="action-bar">
            <div className="action-primary">
              ...
              <button type="button" className="btn-refresh" onClick={fetchAllUniversities}>
          ...
          {!loading && displayData.length > 0 && (
            ...
            {displayData.length > UNIVERSITIES_PER_PAGE && (
              <div className="university-pagination">
                <div className="pagination-count">
                  Showing {pageStartIndex + 1}-{pageEndIndex} of {displayData.length}
                </div>
