...
const GusUniversity = () => {
    ...
    const getDocumentUrl = (meta, label) => {
        if (!meta?.fileName && !meta?.fileUrl) return null;
        if (meta.fileUrl) {
            if (meta.fileUrl.startsWith('/api/files/')) {
                return `${API_BASE_URL}${meta.fileUrl}`;
            } else if (meta.fileUrl.startsWith('/uploads/')) {
                return `${API_BASE_URL}${meta.fileUrl}`;
                ...
                } catch { return meta.fileUrl; }
            } else {
                return `${API_BASE_URL}${meta.fileUrl}`;
            }
        }
        const folder = DOC_FOLDER_MAP_VIEW[label] || 'documents/other';
        return `${API_BASE_URL}/api/files/${folder}/${meta.fileName}`;
    };
    ...
    const handleDownloadMeta = async (meta, label) => {
        const url = getDocumentUrl(meta, label);
        if (!url) return;
        setDownloadingDoc(true);
        try {
            ...
            const a       = document.createElement('a');
            a.href        = blobUrl;
            a.download    = meta.originalName || meta.fileName || label || 'certificate';
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
            URL.revokeObjectURL(blobUrl);
        } catch {
            alert('Download failed. Try "View" and save from there.');
        } finally {
            setDownloadingDoc(false);
        }
    };
    ...
    const renderEqheCertificate = (cert, label) => {
        if (!cert?.uploaded) {
            return (
                <div className="gus-modal-field gus-field-full">
                    <span className="gus-field-lbl">{label}</span>
                    <span className="gus-field-val">No certificate uploaded</span>
                </div>
            );
        }
        ...
        const meta = {
            ...
            originalName: cert.fileName || 'certificate.pdf',
            fileType: cert.fileType || (cert.fileName?.split('.').pop() || 'pdf'),
        ...
        return (
            <div className="gus-modal-field gus-field-full">
                ...
                <span className="gus-field-val">
                    Uploaded: {val(meta.fileName)}
                    <span className="gus-doc-actions-inline">
                        {url && (
                            <a href={url} target="_blank" rel="noopener noreferrer" className="gus-doc-view-btn">
                                View
                            </a>
                        )}
                        <button className="gus-doc-view-btn" onClick={() => handleDownloadMeta(meta, label)} disabled={downloadingDoc}>
                            Download
                        </button>
                    </span>
                </span>
    ...
    const handleDownloadPDF = async (app) => {
        ...
        try {
            ...
            drawRow([{k:'Date',v:formatDate(app.eqheDate)},{k:'Certificate Uploaded',v:app.eqheCertificate?.uploaded?'Yes':'No'}]);
            drawFullRow('Certificate File',app.eqheCertificate?.uploaded ? app.eqheCertificate?.fileName : 'No certificate uploaded');
            drawRow([{k:'Has Additional EQHE',v:yesNo(app.hasAnotherEQHE)},{k:'',v:''}]);
            if(app.hasAnotherEQHE){drawSectionHeader('Additional EQHE Details',[236,72,153]);drawFullRow('Qualification Title',fmt(eqheTitleMap,app.anotherEqheOriginalTitle));drawRow([{k:'Country',v:fmt(countryMap,app.anotherEqheCountry)},{k:'City',v:app.anotherEqheCity}]);drawRow([{k:'Date',v:formatDate(app.anotherEqheDate)},{k:'Certificate Uploaded',v:app.anotherEqheCertificate?.uploaded?'Yes':'No'}]);drawFullRow('Certificate File',app.anotherEqheCertificate?.uploaded ? app.anotherEqheCertificate?.fileName : 'No certificate uploaded');}
            if(app.documents){
                        ...
                        <div className="gus-modal-body">
                            ...
                            <div className="gus-modal-sec">
                                ...
                                <div className="gus-modal-grid">
                                    ...
                                    <div className="gus-modal-field"><span className="gus-field-lbl">Certificate Status</span><span className="gus-field-val">{selectedApp.eqheCertificate?.uploaded?'Uploaded':'No certificate uploaded'}</span></div>
                                    {renderEqheCertificate(selectedApp.eqheCertificate, 'EQHE Certificate')}
                            ...
                            {selectedApp.hasAnotherEQHE && (
                                <div className="gus-modal-sec">
                                    ...
                                    <div className="gus-modal-grid">
                                        ...
                                        <div className="gus-modal-field"><span className="gus-field-lbl">Certificate Status</span><span className="gus-field-val">{selectedApp.anotherEqheCertificate?.uploaded?'Uploaded':'No certificate uploaded'}</span></div>
                                        {renderEqheCertificate(selectedApp.anotherEqheCertificate, 'Additional EQHE Certificate')}
                                    </div>
                                </div>
                            )}
