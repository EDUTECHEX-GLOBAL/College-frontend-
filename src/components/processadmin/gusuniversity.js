...
const GusUniversity = () => {
    ...
    const DOC_FOLDER_MAP_VIEW = {
        ...
        'Certificate 9th':       'documents/certificates',
        'Certificate 10th':      'documents/certificates',
        'Certificate 11th':      'documents/certificates',
        'Certificate 12th':      'documents/certificates',
        'Test Scores':           'documents/optional',
    ...
    const handleDownloadPDF = async (app) => {
        ...
        try {
            ...
            const C = {
                primary: [108, 99, 255], accent: [67, 184, 156],
                dark: [30, 30, 47], mid: [90, 90, 110], light: [245, 245, 252],
                border: [220, 220, 235], white: [255, 255, 255],
            ...
            const checkPageBreak = (needed = 20) => {
                if (y + needed > PH - 16) { doc.addPage(); drawPageFooter(); y = 18; }
            };
            ...
            const drawProgressBar = (pct) => {
                ...
                if (fillW > 0) { doc.setFillColor(...C.primary); doc.roundedRect(ML, y, fillW, BAR_H, 2, 2, 'F'); }
            ...
            const statusColors = { 'COMPLETED': C.green, 'IN PROGRESS': C.yellow, 'INCOMPLETE': C.red };
            ...
            if (app.documents) {
                ...
                const docList = [
                    { label: 'CV',                   up: app.documents.cvUploaded,         st: app.documents.cvStatus,         meta: app.documents.cvMeta,         exp: null },
                    { label: 'Photo',                 up: app.documents.photoUploaded,      st: app.documents.photoStatus,      meta: app.documents.photoMeta,      exp: null },
                    { label: 'Passport',              up: app.documents.passportUploaded,   st: app.documents.passportStatus,   meta: app.documents.passportMeta,   exp: null },
                    { label: 'Transcript',            up: app.documents.transcriptUploaded, st: app.documents.transcriptStatus, meta: app.documents.transcriptMeta, exp: null },
                    { label: 'Diploma',               up: app.documents.diplomaUploaded,    st: app.documents.diplomaStatus,    meta: app.documents.diplomaMeta,    exp: null },
                    { label: 'Cert 9th',              up: app.documents.cert9thUploaded,    st: app.documents.cert9thStatus,    meta: app.documents.cert9thMeta,    exp: app.documents.cert9thExpectedDate },
                    { label: 'Cert 10th',             up: app.documents.cert10thUploaded,   st: app.documents.cert10thStatus,   meta: app.documents.cert10thMeta,   exp: app.documents.cert10thExpectedDate },
                    { label: 'Cert 11th',             up: app.documents.cert11thUploaded,   st: app.documents.cert11thStatus,   meta: app.documents.cert11thMeta,   exp: app.documents.cert11thExpectedDate },
                    { label: 'Cert 12th',             up: app.documents.cert12thUploaded,   st: app.documents.cert12thStatus,   meta: app.documents.cert12thMeta,   exp: app.documents.cert12thExpectedDate },
                    { label: 'Test Scores',           up: app.documents.testScoresUploaded, st: null, meta: app.documents.testScoresMeta, exp: null },
                    { label: 'Language Proficiency',  up: app.documents.langProfUploaded,   st: null, meta: app.documents.langProfMeta,   exp: null },
                    { label: 'Recommendation Letter', up: app.documents.recLetterUploaded,  st: null, meta: app.documents.recLetterMeta,  exp: null },
                ];
                ...
                col1.forEach(({ label, up, st, exp }) => {
                    checkPageBreak(10);
                    const hasExp = !up && !!exp;
                    const bg  = up ? (st === 'approved' ? C.green : st === 'rejected' ? C.red : C.yellow) : hasExp ? [251, 191, 36] : C.red;
                    ...
                    if (hasExp) {
                        doc.setFontSize(7).setTextColor(...C.mid);
                        doc.text(`Expected: ${formatExpectedDate(exp)}`, ML + 25, y1 + 8.5);
                        y1 += 4;
                    }
                    y1 += 9;
                });

                col2.forEach(({ label, up, st, exp }) => {
                    const hasExp = !up && !!exp;
                    const bg  = up ? (st === 'approved' ? C.green : st === 'rejected' ? C.red : C.yellow) : hasExp ? [251, 191, 36] : C.red;
                    const txt = up ? (st === 'approved' ? 'Approved' : st === 'rejected' ? 'Rejected' : 'Pending') : hasExp ? 'Expected' : 'Missing';
                    doc.setFillColor(...bg);
                    ...
                    if (hasExp) {
                        doc.setFontSize(7).setTextColor(...C.mid);
                        doc.text(`Expected: ${formatExpectedDate(exp)}`, col2X + 25, y2 + 8.5);
                        y2 += 4;
                    }
                    y2 += 9;
                });
                ...
                y = Math.max(y1, y2) + 4;
                if (app.documents.portfolioLink) drawFullRow('Portfolio Link', app.documents.portfolioLink);
            }
            ...
            if (app.scores && Object.values(app.scores).some(v => v && typeof v === 'string')) {
                ...
                for (let i = 0; i < scoreList.length; i += 2) drawRow([scoreList[i], scoreList[i + 1] || { k: '', v: '' }]);
            }
            ...
            if (app.specialNeeds) {
                ...
                if (app.specialNeeds.hasSpecialNeeds === 'yes') {
                    if (app.specialNeeds.specialNeeds?.length)        drawFullRow('Special Needs', app.specialNeeds.specialNeeds.join(', '));
                    if (app.specialNeeds.requiredArrangements?.length) drawFullRow('Required Arrangements', app.specialNeeds.requiredArrangements.join(', '));
                }
            }
            ...
            // ══════════════════════════════════════════════════════
            // APPEND UPLOADED DOCUMENTS AS PAGES
            // ══════════════════════════════════════════════════════
            if (app.documents) {
                const docEntries = [
                    { label: 'CV',                    meta: app.documents.cvMeta         },
                    { label: 'Photo',                 meta: app.documents.photoMeta      },
                    { label: 'Passport',              meta: app.documents.passportMeta   },
                    { label: 'Transcript',            meta: app.documents.transcriptMeta },
                    { label: 'Diploma',               meta: app.documents.diplomaMeta    },
                    { label: 'Certificate 9th',       meta: app.documents.cert9thMeta    },
                    { label: 'Certificate 10th',      meta: app.documents.cert10thMeta   },
                    { label: 'Certificate 11th',      meta: app.documents.cert11thMeta   },
                    { label: 'Certificate 12th',      meta: app.documents.cert12thMeta   },
                    { label: 'Test Scores',           meta: app.documents.testScoresMeta },
                    { label: 'Language Proficiency',  meta: app.documents.langProfMeta   },
                ...
                const addDocHeader = (lbl, filename, pageNum = null, totalPages = null) => {
                    ...
                    doc.setFontSize(11).setFont(undefined, 'bold').setTextColor(...C.white);
                    const pageInfo = pageNum ? `  ·  Page ${pageNum}/${totalPages}` : '';
                    doc.text(`${lbl}${pageInfo}`, ML, 7);
                    doc.setFontSize(8).setFont(undefined, 'normal').setTextColor(200, 195, 255);
                    const maxLen = 80;
                    const fname  = (filename || '').length > maxLen
                        ? (filename || '').slice(0, maxLen) + '…'
                        : (filename || '');
                ...
                const embedImage = (base64, fmt, label, filename, pageNum = null, totalPages = null) => {
                    ...
                    const xOff = ML + (maxW - drawW) / 2;
                    doc.addImage(base64, fmt, xOff, 19, drawW, drawH, undefined, 'MEDIUM');
                };
                ...
                for (const { label, meta } of docEntries) {
                    try {
                        ...
                        console.warn(`⚠️ Could not embed document "${label}":`, docErr.message);
                        doc.addPage();
                        drawPageFooter();
                        addDocHeader(label, meta.originalName || meta.fileName || '');
                        doc.setFontSize(13).setFont(undefined, 'bold').setTextColor(...C.mid);
                        doc.text('⚠️  Could not load document preview.', PW / 2, PH / 2 - 8, { align: 'center' });
                        doc.setFontSize(8).setFont(undefined, 'normal').setTextColor(...C.mid);
                        doc.text(meta.fileUrl || '', PW / 2, PH / 2, { align: 'center' });
                        doc.setFontSize(8).setTextColor(...C.mid);
                        doc.text(`Error: ${docErr.message}`, PW / 2, PH / 2 + 8, { align: 'center' });
                    }
                }
            }
                        ...
                        {filtered.length === 0 ? (
                            <tr><td colSpan="6">
                                <div className="gus-empty-state">
                                    ...
                                    <p>{applications.length === 0 ? 'No applications found in the database.' : 'No applications match your search or filter.'}</p>
                                </div>
                            </td></tr>
                        ) : (
                                ...
                                <tr key={app._id || idx} className="gus-tbody-row" style={{ animationDelay: `${idx * 0.04}s` }}>
                                    <td>
                                        <div>
                                            ...
                                            <div style={{ fontSize: '11px', color: '#888', marginTop: 2 }}>{app.studentId?.slice(-8)}</div>
                                        </div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div className="gus-action-group">
                                            ...
                                            <button className="gus-btn-dl" onClick={() => handleDownloadPDF(app)} title="Download PDF" disabled={pdfLoading === app.studentId}>
                                                {pdfLoading === app.studentId ? <span className="gus-btn-spinner" /> : (
                                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="gus-btn-ico">
                                                    ...
                                                    <div className="gus-doc-card-body">
                                                        ...
                                                        {up && meta && (
                                                            <div className="gus-doc-file-info">
                                                                ...
                                                                <button
                                                                    className="gus-doc-view-btn"
                                                                    onClick={() => openDocViewer(meta, lbl)}
                                                                >
                        ...
                        <div className="gus-docviewer-body">
                            {isImageType(docViewer.fileType) ? (
                                <div className="gus-docviewer-img-wrap">
                                    <img
                                        src={docViewer.url}
                                        alt={docViewer.originalName}
                                        className="gus-docviewer-img"
                                        onError={e => {
