...
const GusUniversity = () => {
    ...
    const formatGender = (s) => {
        ...
        return labels[normalized] || 'Not Provided';
    };
    ...
    const getUniversityName = (app = {}) =>
        pickDisplayValue(
            app.applicationUniversityName,
            app.completedUniversityName,
            app.selectedUniversity?.name,
            app.selectedUniversity?.universityName,
            app.selectedCourse?.universityName,
            app.selectedCourse?.university,
            app.universityName,
            app.university
        );
    ...
    const getProgramName = (app = {}) =>
        pickDisplayValue(
            app.applicationProgramName,
            app.completedProgramName,
            app.selectedCourse?.programName,
            app.selectedCourse?.title,
            app.selectedCourse?.name,
            app.programName
        );
    ...
    const getPermanentAddress = (address = {}) =>
        address.permanentAddress || formatAddressLine([
    ...
    const getCorrespondenceAddress = (address = {}) => {
        if (!address.hasDifferentCorrespondenceAddress) return getPermanentAddress(address);
        return formatAddressLine([
            address.correspondenceCareOf ? `c/o ${address.correspondenceCareOf}` : '',
    ...
    const fetchApplicationDetails = useCallback(async (studentId) => {
        const res = await api.get(`/api/application/process-admin/gus-university/applications/${studentId}`);
        return res.data;
    }, [api]);
    ...
    const openModal = async (app) => {
        ...
        setSelectedApp({ ...app, _loadingDetails: true });

        try {
            const res = await fetchApplicationDetails(app.studentId);
            if (res.success) {
                setSelectedApp(res.data);
            } else {
                setSelectedApp({ ...app, _detailsError: true });
            }
        } catch (err) {
    ...
    const toggleModalSection = (section) => {
        setExpandedModalSections(prev => ({ ...prev, [section]: !prev[section] }));
    };

    const getDocumentUrl = (meta, label) => {
        if (!meta?.fileName && !meta?.fileUrl && !meta?.fileKey) return null;
        if (meta.fileKey || meta.fileUrl) return resolveFileUrl(meta.fileKey || meta.fileUrl);
        return null;
    };

    const openDocViewer = (meta, label) => {
        const url = getDocumentUrl(meta, label);
        if (!url) return;
    ...
    const canVerifyApplication = (app = {}) => {
        if (!app || app.processAdminVerified) return false;
        const status = String(app.overviewStatus || app.applicationStatus || app.status || '').toLowerCase();
        return app.completionPercentage === 100 || ['submitted', 'completed'].includes(status) || app.isCompleted === true;
    };
    ...
    const submitVerification = async () => {
        ...
        setVerifyError('');
        try {
            await api.post('/api/process-admin/verify-application', {
                applicationId: verifyModal.applicationId,
                studentId: verifyModal.studentId,
            });
            setVerifyModal(null);
            await fetchApplications();
        } catch (err) {
            setVerifyError(err.response?.data?.message || 'Failed to send application to Admin Verification Queue.');
        } finally {
    ...
    const handleSendDocEmail = async () => {
        ...
        try {
            ...
            const response = await fetch(
                `${API_BASE_URL}/api/application/process-admin/gus-university/send-doc-email`,
                {
                    method: 'POST',
                    headers: {
    ...
    const handleDownloadPDF = async (appSummary) => {
        setPdfLoading(appSummary.studentId);
        try {
            let appForPdf = appSummary;
            if (!appForPdf.documents && appForPdf.studentId) {
                const detailRes = await fetchApplicationDetails(appForPdf.studentId);
                if (detailRes.success) appForPdf = detailRes.data;
            }

            console.log("PDF university:", getUniversityName(appForPdf), appForPdf);
            const JsPDF = await loadJsPDF();
            const doc   = new JsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
            const app = appForPdf;
            const resolvedUniversityName = getUniversityName(app);
            const pdfUniversityName = resolvedUniversityName && resolvedUniversityName !== '—' ? resolvedUniversityName : "University Application";
            ...
            let y=0;
            const checkPageBreak=(needed=20)=>{if(y+needed>PH-22){drawPageFooter();doc.addPage();y=18;}};
            const drawPdfFooter=()=>{const pg=doc.internal.getCurrentPageInfo().pageNumber;doc.setFontSize(8).setTextColor(...C.mid);doc.text(`${pdfUniversityName} — ${app.applicationId||'Application'}`,ML,PH-8);doc.text(`Page ${pg}`,PW-ML,PH-8,{align:'right'});doc.setDrawColor(...C.border);doc.line(ML,PH-12,PW-MR,PH-12);};
            void drawPdfFooter;
            const drawPageFooter=()=>{const pg=doc.internal.getCurrentPageInfo().pageNumber;const footerText=`${pdfUniversityName} — ${app.applicationId||"Application"}`;const safeFooterText=doc.splitTextToSize(footerText,120)[0];doc.setDrawColor(...C.border);doc.line(ML,PH-12,PW-MR,PH-12);doc.setFontSize(7);doc.setTextColor(...C.mid);doc.text(safeFooterText,ML,PH-8,{maxWidth:120});doc.text(`Page ${pg}`,PW-MR,PH-8,{align:"right"});};
            ...
            drawRow([{k:'Gender',v:formatGender(app.gender)},{k:'Date of Birth',v:formatDate(app.dateOfBirth)}]);
            ...
            if(app.address){
                drawSectionHeader('Address Information',C.accent);
                drawFullRow('Permanent Address',getPermanentAddress(app.address));
                drawFullRow('Correspondence Address',getCorrespondenceAddress(app.address));
                drawRow([{k:'Care Of',v:app.address.careOf},{k:'City',v:app.address.city}]);
                drawRow([{k:'State',v:app.address.stateProvince},{k:'Country',v:app.address.country}]);
                drawRow([{k:'Postcode',v:app.address.postcode},{k:'Address Completed',v:app.address.isCompleted?'Yes':'No'}]);
            }
            ...
            checkPageBreak(20);doc.setFontSize(8).setFont(undefined,'normal').setTextColor(...C.mid);doc.text('Application Completion:',ML,y);y+=5;drawProgressBar(app.completionPercentage||0);drawPageFooter();
        ...
        <div className="gus-wrap">
            ...
            <div className="gus-page-header">
                ...
                <div className="gus-page-header-left">
                    <h1 className="gus-page-title">Bachelors Applications</h1>
                                    ...
                                    <tr key={app._id||idx} className="gus-tbody-row" style={{animationDelay:`${idx*0.04}s`}}>
                                        ...
                                        <td>
                                            <div className="gus-action-group">
                                                ...
                                                {app.processAdminVerified ? (
                                                    <span className="gus-sent-badge">Sent to Admin</span>
                                                ) : canVerifyApplication(app) ? (
                                                    <button className="gus-btn-verify" onClick={() => { setVerifyError(''); setVerifyModal(app); }}>Verify</button>
                                                ) : null}
                                            </div>
                                        </td>
                                    </tr>
            ...
            {verifyModal && (
                <div className="gus-overlay" onClick={() => !verifyLoading && setVerifyModal(null)}>
                    <div className="gus-verify-modal" onClick={event => event.stopPropagation()}>
                        <div className="gus-verify-header">
                            <div><h3>Verify Application</h3><p>Send this completed application to the Admin Verification Queue.</p></div>
                            <button type="button" onClick={() => setVerifyModal(null)} disabled={verifyLoading}>&times;</button>
                        </div>
                        <div className="gus-verify-details">
                            <div><span>Student</span><strong>{verifyModal.studentName || 'Unknown'}</strong></div>
                            <div><span>Application ID</span><strong>{verifyModal.applicationId || '-'}</strong></div>
                            <div><span>University</span><strong>{getUniversityName(verifyModal)}</strong></div>
                            <div><span>Programme</span><strong>{getProgramName(verifyModal)}</strong></div>
                        </div>
                        <p className="gus-verify-message">This will send the application and uploaded documents to Admin Verification Queue.</p>
                        {verifyError && <div className="gus-verify-error">{verifyError}</div>}
                        <div className="gus-verify-actions">
                            <button type="button" className="gus-verify-cancel" onClick={() => setVerifyModal(null)} disabled={verifyLoading}>Cancel</button>
                            <button type="button" className="gus-verify-submit" onClick={submitVerification} disabled={verifyLoading}>{verifyLoading ? 'Sending...' : 'Verify & Send'}</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="gus-overlay" onClick={closeModal}>
                    <div className="gus-modal" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="gus-modal-body">
                            {selectedApp?._loadingDetails && (
                                <div className="gus-modal-loading">
                                    Loading application details...
                                </div>
                            )}

                            {selectedApp?._detailsError && (
                                <div className="gus-modal-loading">
                                    Failed to load application details. Please close and try again.
                                </div>
                            )}

                            {!selectedApp?._loadingDetails && !selectedApp?._detailsError && (
                                <>
                            ...
                            <div className="gus-modal-sec">
                                ...
                                <div className="gus-modal-grid">
                                    ...
                                    <div className="gus-modal-field"><span className="gus-field-lbl">Gender</span><span className="gus-field-val">{formatGender(selectedApp.gender)}</span></div>
                            ...
                            {selectedApp.address && (
                                <div className="gus-modal-sec">
                                    <div className="gus-modal-sec-title"><span className="gus-sec-dot gus-dot-blue"/>Address Information</div>
                                    <div className="gus-modal-grid">
                                        <div className="gus-modal-field gus-field-full"><span className="gus-field-lbl">Permanent Address</span><span className="gus-field-val">{getPermanentAddress(selectedApp.address)}</span></div>
                                        <div className="gus-modal-field gus-field-full"><span className="gus-field-lbl">Correspondence Address</span><span className="gus-field-val">{getCorrespondenceAddress(selectedApp.address)}</span></div>
                                        <div className="gus-modal-field"><span className="gus-field-lbl">Care Of</span><span className="gus-field-val">{val(selectedApp.address?.careOf)}</span></div>
                                        <div className="gus-modal-field"><span className="gus-field-lbl">City</span><span className="gus-field-val">{val(selectedApp.address?.city)}</span></div>
                                        <div className="gus-modal-field"><span className="gus-field-lbl">State</span><span className="gus-field-val">{val(selectedApp.address?.stateProvince)}</span></div>
                                        <div className="gus-modal-field"><span className="gus-field-lbl">Country</span><span className="gus-field-val">{val(selectedApp.address?.country)}</span></div>
                                        <div className="gus-modal-field"><span className="gus-field-lbl">Postcode</span><span className="gus-field-val">{val(selectedApp.address?.postcode)}</span></div>
                                        <div className="gus-modal-field"><span className="gus-field-lbl">Different Correspondence Address</span><span className="gus-field-val">{yesNo(selectedApp.address?.hasDifferentCorrespondenceAddress)}</span></div>
                                        {selectedApp.address?.hasDifferentCorrespondenceAddress && (
                                            <>
                                                <div className="gus-modal-field"><span className="gus-field-lbl">Correspondence Care Of</span><span className="gus-field-val">{val(selectedApp.address?.correspondenceCareOf)}</span></div>
                                                <div className="gus-modal-field"><span className="gus-field-lbl">Correspondence City</span><span className="gus-field-val">{val(selectedApp.address?.correspondenceCity)}</span></div>
                                                <div className="gus-modal-field"><span className="gus-field-lbl">Correspondence State</span><span className="gus-field-val">{val(selectedApp.address?.correspondenceStateProvince)}</span></div>
                                                <div className="gus-modal-field"><span className="gus-field-lbl">Correspondence Country</span><span className="gus-field-val">{val(selectedApp.address?.correspondenceCountry)}</span></div>
                                                <div className="gus-modal-field"><span className="gus-field-lbl">Correspondence Postcode</span><span className="gus-field-val">{val(selectedApp.address?.correspondencePostcode)}</span></div>
                                            </>
                                        )}
                                    </div>
                                </div>
                            ...
                            {selectedApp.documents && (
                                <div className="gus-modal-sec">
                                    <button
                                        type="button"
                                        className="gus-modal-sec-title"
                                        onClick={() => toggleModalSection('documents')}
                                    >
                                        <span className="gus-sec-dot gus-dot-purple"/>
                                        Documents Status
                                        <span style={{marginLeft:'auto'}}>{expandedModalSections.documents ? '−' : '+'}</span>
                                    </button>
                                    {expandedModalSections.documents && (
                                        <>
