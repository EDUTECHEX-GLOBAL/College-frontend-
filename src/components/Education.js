...
const normalizeOtherSchools = (otherSchools = {}) => {
  const schools = Array.isArray(otherSchools.schools) ? otherSchools.schools : [];
  const parsedCount = Number.parseInt(otherSchools.numberOfSchools, 10);
  const count = Number.isFinite(parsedCount)
    ? Math.max(0, Math.min(3, parsedCount))
    : Math.min(3, schools.length);

  return {
    ...otherSchools,
    numberOfSchools: count,
    schools: Array.from({ length: count }, (_, index) => ({
      ...createEmptyOtherSchool(),
      ...(schools[index] || {})
    }))
  };
};
...
const normalizeCurrentCourses = (currentCourses = {}) => {
  const courses = Array.isArray(currentCourses.courses) ? currentCourses.courses : [];
  const parsedCount = Number.parseInt(currentCourses.numberOfCourses, 10);
  const count = Number.isFinite(parsedCount)
    ? Math.max(0, Math.min(10, parsedCount))
    : Math.min(10, courses.length);
...
const createEmptyCommunityOrganization = () => ({
  organizationName: '',
  assistanceType: '',
  duration: '',
  contactPerson: ''
...
const normalizeCommunityOrganizations = (communityOrganizations = {}) => {
  const organizations = Array.isArray(communityOrganizations.organizations)
    ? communityOrganizations.organizations
    : [];
  const parsedCount = Number.parseInt(
    communityOrganizations.numberOfOrganizations,
    10
  );
  const count = Number.isFinite(parsedCount)
    ? Math.max(0, Math.min(3, parsedCount))
    : Math.min(3, organizations.length);

  return {
    ...communityOrganizations,
    numberOfOrganizations: count,
    organizations: Array.from({ length: count }, (_, index) => ({
      ...createEmptyCommunityOrganization(),
      ...(organizations[index] || {})
    }))
  };
};
...
const EducationForm = () => {
    ...
    try {
      ...
      if (response.data.success && response.data.education) {
        setEducationData({
          ...
          otherSchools: normalizeOtherSchools(response.data.education.otherSchools),
          currentCourses: normalizeCurrentCourses(response.data.education.currentCourses),
          communityOrganizations: normalizeCommunityOrganizations(
            response.data.education.communityOrganizations
          )
        });
  ...
  const handleInputChange = (section, field, value) => {
    setEducationData(prev => {
      ...
      if (section === 'otherSchools' && field === 'numberOfSchools') {
        updatedSection = normalizeOtherSchools(updatedSection);
      }
      if (section === 'currentCourses' && field === 'numberOfCourses') {
        updatedSection = normalizeCurrentCourses(updatedSection);
      }
      if (section === 'communityOrganizations' && field === 'numberOfOrganizations') {
        updatedSection = normalizeCommunityOrganizations(updatedSection);
      }
  ...
  const addArrayItem = (section, arrayField, newItem) => {
    setEducationData(prev => {
      const updatedArray = [...prev[section][arrayField], newItem];
      return {
        ...prev,
        [section]: {
          ...prev[section],
          [arrayField]: updatedArray,
          ...(section === 'otherSchools' && arrayField === 'schools'
            ? { numberOfSchools: updatedArray.length }
            : {}),
          ...(section === 'currentCourses' && arrayField === 'courses'
            ? { numberOfCourses: updatedArray.length }
            : {}),
          ...(section === 'communityOrganizations' && arrayField === 'organizations'
            ? { numberOfOrganizations: updatedArray.length }
            : {})
        }
      };
    });
  ...
  const removeArrayItem = (section, arrayField, index) => {
    setEducationData(prev => {
      const updatedArray = prev[section][arrayField].filter((_, i) => i !== index);
      return {
        ...prev,
        [section]: {
          ...prev[section],
