...
/* ── Success Modal ── */
const SuccessModal = ({ applicationId, loginEmail, onClose }) => (
...
/* ── Resend Email Modal ── */
const ResendEmailModal = ({ applicationId, loginEmail, onClose }) => {
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      const res   = await fetch(`${API_BASE_URL}/api/master-preview/resend-email`, {
        method:  'POST',
...
const MasterPreview = ({ data, updateData, onEdit, onSubmitSuccess }) => {
  ...
  useEffect(() => {
    ...
    const fetchPreview = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/api/master-preview`, {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/master-preview/submit`, {
        method: 'POST',
        headers: {
      ...
      // FIX: handle non-OK status before trying to parse JSON
      if (!res.ok) {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/master-preview/download-pdf`, {
        headers: { Authorization: `Bearer ${token}` },
      });

      if (!res.ok) {
        const errData = await res.json().catch(() => ({}));
        alert(errData.message || 'Failed to download PDF. Please try again.');
        return;
      }
