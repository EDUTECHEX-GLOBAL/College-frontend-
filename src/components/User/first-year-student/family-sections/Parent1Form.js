...
const Parent1Form = () => {
  ...
  const [formData, setFormData] = useState(parentDefaults);
  ...
  const fetchParent1Data = async () => {
    try {
      ...
      if (response.data.success && response.data.familyData.parent1) {
        const nextData = normalizeParentData(response.data.familyData.parent1);
        setFormData(nextData);
        setErrors(getParentErrors(nextData, { requireAll: false }));
      }
    } catch (error) {
  ...
  const updateData = (nextData) => {
    setFormData(nextData);
    const nextErrors = getParentErrors(nextData, { requireAll: false });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const finalErrors = getParentErrors(formData, { requireAll: true });
    ...
    try {
      await axiosInstance.post('/api/students/family-dashb/parent1', payload);
      ...
      alert(error.response?.data?.message || 'Error saving parent 1 data. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="family-form-container">
      ...
      <form onSubmit={handleSubmit} className="family-form parent1-form" noValidate>
        <div className="form-field">
          <label className="form-label required">Parent 1 type</label>
        ...
        {formData.parentType !== 'limited_info' && (
          <>
            <div className="form-field">
              <label className="form-label">Is parent 1 living?</label>
