...
const API_URL  = `${API_BASE_URL}/api/master-documents`;
...
const DOCUMENT_TYPES = [
  ...
  },
  {
    ...
    required: true, accept: '.pdf,.jpg,.jpeg,.png', maxSize: 5, category: 'professional',
  },
  {
...
const MasterDocuments = ({ data, updateData }) => {
          ...
          DOCUMENT_TYPES.forEach(({ field }) => {
            if (doc[field]?.fileName) {
              mapped[field] = {
                ...
                fileUrl:      resolveFileUrl(doc[field].fileKey || doc[field].fileUrl),
    ...
    try {
      ...
      if (result.success && result.fileData) {
        const uploaded = {
          ...
          fileUrl:      resolveFileUrl(result.fileData.fileKey || result.fileData.fileUrl),
  ...
  const UploadArea = ({ doc }) => {
    ...
    const viewUrl   = resolveFileUrl(fileData?.fileKey || fileData?.fileUrl);
