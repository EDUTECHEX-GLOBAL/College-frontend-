...
const isValidMasterContact = (contact = {}) => (
...
const Master = ({ onUpdate }) => {
    ...
    const buildInitialFormData = () => {
        ...
        const scopedSaved = currentUserId ? safeGetLocalStorage(getMasterApplicationStorageKey(currentUserId)) || {} : {};
        const saved = { ...legacySaved, ...scopedSaved };
    ...
    const saveToLocalStorage = useCallback(() => {
        ...
        if (studentId) {
            localStorage.setItem(getMasterApplicationStorageKey(studentId), JSON.stringify(formData));
        }
        const userDataStr = localStorage.getItem('userData');
    ...
    }, [formData, calculateProgress, onUpdate, studentId]);
