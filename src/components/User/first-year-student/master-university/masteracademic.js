...
const UNIVERSITY_REGEX   = /^[A-Za-z][A-Za-z\s.,'&-]{1,100}$/;
const FIELD_OF_STUDY_REGEX = /^[A-Za-z][A-Za-z\s&-]{1,80}$/;
const GPA_FORMAT_REGEX    = /^(\d+(\.\d+)?|\d+(\.\d+)?\/\d+(\.\d+)?|\d+(\.\d+)?%)$/;

const sanitizeUniversity = (value) =>
  value.replace(/[^A-Za-z\s.,'&-]/g, '').slice(0, 101);

const sanitizeFieldOfStudy = (value) =>
  value.replace(/[^A-Za-z\s&-]/g, '').slice(0, 81);

const sanitizeGpa = (value) =>
  value.replace(/[^0-9./%]/g, '');

const isValidUniversity = (value) => UNIVERSITY_REGEX.test(value.trim());

const isValidFieldOfStudy = (value) => FIELD_OF_STUDY_REGEX.test(value.trim());
...
const MasterAcademic = ({ data, updateData }) => {
  ...
  const isValidGpa = (gpaStr) => {
    ...
    if (!GPA_FORMAT_REGEX.test(cleaned)) return false;
    if (cleaned.endsWith('%')) {
  ...
  const validateEntry = useCallback((entry, entryId, showAll = false) => {
    ...
    if ((showAll || touched.university) && entry.university?.trim() && !isValidUniversity(entry.university)) {
      newErrors.university = 'University name must contain letters and cannot be only numbers';
    }

    if ((showAll || touched.fieldOfStudy) && entry.fieldOfStudy?.trim() && !isValidFieldOfStudy(entry.fieldOfStudy)) {
      newErrors.fieldOfStudy = 'Field of study must contain only letters';
    }
    ...
    if ((showAll || touched.gpa) && entry.gpa?.trim() && !isValidGpa(entry.gpa)) {
      newErrors.gpa = 'Enter a valid GPA or percentage';
    }
  ...
  const checkIsValid = useCallback((entries) => {
    const allFilled = entries.every(e =>
      e.degree?.trim() &&
      e.university?.trim() &&
      isValidUniversity(e.university) &&
      e.country?.trim() &&
      e.fieldOfStudy?.trim() &&
      isValidFieldOfStudy(e.fieldOfStudy) &&
      e.startDate &&
      e.endDate &&
  ...
  const fetchAndPopulate = useCallback(async (signal) => {
    ...
    const response = await fetch(`${API_BASE_URL}/api/master-academic`, {
      method:  'GET',
  ...
  const handleEntryChange = useCallback((id, field, value) => {
    ...
    if (field === 'university') sanitizedValue = sanitizeUniversity(value);
    if (field === 'fieldOfStudy') sanitizedValue = sanitizeFieldOfStudy(value);
    if (field === 'gpa') sanitizedValue = sanitizeGpa(value);
    ...
    setAcademicEntries(prev => {
      const updated = prev.map(entry => entry.id === id ? { ...entry, [field]: sanitizedValue } : entry);
