...
const getDisplayFileName = (file) =>
    file?.originalName || file?.fileName || file?.name || 'Uploaded certificate';
...
const ApplicationLanguage = ({ formData, onInputChange, onFileUpload, studentId, onNext }) => {
    ...
    const [openEqhePicker, setOpenEqhePicker]         = useState(null);
    ...
    useEffect(() => {
        const fetchExistingData = async () => {
            ...
            try {
                ...
                if (response.data.success) {
                    ...
                    if (data.eqheCertificate) {
                        onFileUpload('eqheCertificate', {
                            ...data.eqheCertificate,
                            fileUrl: resolveFileUrl(data.eqheCertificate.fileKey || data.eqheCertificate.fileUrl || data.eqheCertificateUrl),
                            fileName: data.eqheCertificate.fileName || data.eqheCertificateFileName,
                            originalName: data.eqheCertificate.originalName || data.eqheCertificateFileName,
                            size: data.eqheCertificate.size || data.eqheCertificateFileSize,
                        });
                    }
        ...
        fetchExistingData();
    }, [studentId, mapToResumeFields, onInputChange, onFileUpload]);
    ...
    const handleUploadCertificate = async (file, certificateType = 'eqheCertificate') => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                onFileUpload(certificateType, {
                    ...uploadedFile,
                    fileUrl: resolveFileUrl(uploadedFile.fileUrl),
                    name: uploadedFile.originalName || uploadedFile.fileName || file.name,
    ...
    const getEqheTitleLabel = (value) =>
        eqheTitles.find(title => title.value === value)?.label || '';

    const getCountryLabel = (value) =>
        countries.find(country => country.value === value)?.label || '';

    const handleEqheTitleSelect = (field, value, mirrorEnglishTestType = false) => {
        onInputChange(field, value);
        if (mirrorEnglishTestType) onInputChange('englishTestType', value);
        if (validationErrors[field]) {
            setValidationErrors(prev => ({ ...prev, [field]: null }));
        }
        setOpenEqhePicker(null);
    };

    const handleCountrySelect = (field, value, mirrorCountryOfStudy = false) => {
        onInputChange(field, value);
        if (mirrorCountryOfStudy) onInputChange('countryOfStudy', value);
        if (validationErrors[field]) {
            setValidationErrors(prev => ({ ...prev, [field]: null }));
        }
        setOpenEqhePicker(null);
    ...
    const renderCountryPicker = ({ id, value, errorKey, mirrorCountryOfStudy = false }) => {
        const isOpen = openEqhePicker === id;
        const selectedLabel = getCountryLabel(value);

        return (
            <div
                className="applicationlanguage-compact-select"
                onBlur={(e) => {
            ...
            >
                <button
                    type="button"
                    id={id}
                    className={`applicationlanguage-compact-select-trigger ${validationErrors[errorKey] ? 'error' : ''}`}
                    aria-haspopup="listbox"
                    aria-expanded={isOpen}
                    onClick={() => setOpenEqhePicker(isOpen ? null : id)}
                >
                    <span>{selectedLabel || 'Select Country'}</span>
                    <span className="applicationlanguage-compact-select-arrow">v</span>
                </button>

                {isOpen && (
                    <div
                        className="applicationlanguage-compact-select-list"
                        role="listbox"
                        aria-labelledby={id}
                    >
                        {countries.map(country => (
                            <button
                                type="button"
                                key={country.value}
                                className={`applicationlanguage-compact-select-option ${value === country.value ? 'selected' : ''}`}
                                role="option"
                                aria-selected={value === country.value}
                                onClick={() => handleCountrySelect(errorKey, country.value, mirrorCountryOfStudy)}
                            >
                                {country.label}
                            </button>
                        ))}
    ...
    const renderEqheTitlePicker = ({ id, value, errorKey, mirrorEnglishTestType = false }) => {
        const isOpen = openEqhePicker === id;
        const selectedLabel = getEqheTitleLabel(value);
        ...
        return (
            ...
            >
                <button
                    ...
                    onClick={() => setOpenEqhePicker(isOpen ? null : id)}
                >
                    <span>{selectedLabel || 'Select EQHE Title'}</span>
                    <span className="applicationlanguage-compact-select-arrow">v</span>
                </button>
                    ...
                    >
                        {eqheTitles.map(title => (
                            <button
                                type="button"
                                key={title.value}
                                className={`applicationlanguage-compact-select-option ${value === title.value ? 'selected' : ''}`}
                                role="option"
                                aria-selected={value === title.value}
                                onClick={() => handleEqheTitleSelect(errorKey, title.value, mirrorEnglishTestType)}
                            >
                                {title.label}
                            </button>
                        ))}
                ...
                <div className="applicationlanguage-form-card">
                    ...
                    <div className="applicationlanguage-form-section">
                        ...
                        <div className="applicationlanguage-form-grid">
                            ...
                            <div className="applicationlanguage-form-group">
                                ...
                                {renderCountryPicker({
                                    ...
                                    errorKey: 'eqheCountry',
                                    mirrorCountryOfStudy: true,
                                })}
                                {validationErrors.eqheCountry && (
                                    <div className="applicationlanguage-field-error">
                                        {validationErrors.eqheCountry}
                        ...
                        <div className="applicationlanguage-form-group full-width">
                            ...
                            {renderEqheTitlePicker({
                                id: 'eqheOriginalTitle',
                                value: formData.eqheOriginalTitle || '',
                                errorKey: 'eqheOriginalTitle',
                                mirrorEnglishTestType: true,
                            })}
                            {validationErrors.eqheOriginalTitle && (
                                <div className="applicationlanguage-field-error">
                                    {validationErrors.eqheOriginalTitle}
                            ...
                            <div className="applicationlanguage-form-grid">
                                ...
                                <div className="applicationlanguage-form-group full-width">
                                    ...
                                    {renderEqheTitlePicker({
                                        ...
                                        errorKey: 'anotherEqheOriginalTitle',
                                    })}
                                    {validationErrors.anotherEqheOriginalTitle && (
                                        <div className="applicationlanguage-field-error">
                                            {validationErrors.anotherEqheOriginalTitle}
                                    ...
                                    <div className="applicationlanguage-file-preview">
                                        <div className="applicationlanguage-file-info">
                                            <div className="applicationlanguage-file-details">
                                                <span className="applicationlanguage-file-name">
                                                    {getDisplayFileName(formData.eqheCertificate)}
                                                </span>
                                                <span className="applicationlanguage-file-size">
                                                    {getDisplayFileSize(formData.eqheCertificate)}
                                                </span>
                                            </div>
                                        </div>
                                        <div className="applicationlanguage-file-actions">
                                            <button
                                                ...
                                                onClick={() => {
                                                    const url = resolveFileUrl(formData.eqheCertificate.fileKey || formData.eqheCertificate.fileUrl);
