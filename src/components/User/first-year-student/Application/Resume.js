...
const getDefaultMaxMarks = (subject = '') => {
  if (SCALE_7_SUBJECT_PATTERN.test(subject)) return 7;
  if (/math|mathematics/i.test(subject) || LANGUAGE_SUBJECT_PATTERN.test(subject)) return 150;
  if (PERCENTAGE_SUBJECT_PATTERN.test(subject)) return 100;
  return 60;
};

const normalizeSubjectMark = (entry, subject = '') => {
  ...
  return { marks: entry ?? '', maxMarks: getDefaultMaxMarks(subject) };
};
...
const calcAvg = (subjects = []) => {
  ...
  return (valid.reduce((a, b) => {
    const maxMarks = Number(b.maxMarks) || getDefaultMaxMarks(b.subject);
    return a + ((parseFloat(b.marks) / maxMarks) * 100);
...
const parseScores = (r) => {
  ...
  GRADE_KEYS.forEach((gk) => {
    ...
    grades[gk] = subjectList.map(s => ({ subject: s, ...normalizeSubjectMark(marksMap[s], s) }));
  });
    ...
    <>
            ...
            return (
              <div key={gk} className="entry-block score-grade-block" style={{ marginBottom: 14 }}>
                ...
                <div className="score-subject-table">
                  ...
                  {subjects.map(({ subject, marks, maxMarks }) => {
                    const num = parseFloat(marks);
                    const outOf = Number(maxMarks) || getDefaultMaxMarks(subject);
                    const pct = marks === '' || isNaN(num) ? null : (num / outOf) * 100;
                    const grade = pct === null ? '—' : pct >= 90 ? 'A+' : pct >= 80 ? 'A' : pct >= 70 ? 'B+' : pct >= 60 ? 'B' : pct >= 50 ? 'C' : 'F';
                    const colorClass = pct === null ? '' : pct >= 80 ? 'grade-hi' : pct >= 60 ? 'grade-mid' : 'grade-lo';
                    return (
                      <div key={subject} className="score-subject-row">
                        <span className="score-subject-name">{subject}</span>
                        <span className="score-subject-marks">{marks !== '' ? `${marks}/${outOf}` : '—'}</span>
      ...
      <div className="cv-actions no-print">
        ...
        <button
          ...
          onClick={uploadStatus === 'error' ? handleRetryUpload : handleDownloadAndUpload}
    ...
    <div className="rv-wrapper">
      ...
      <div className="rv-toolbar no-print">
        ...
        <div className="toolbar-r">
          ...
          >
            {isProcessing ? '…' : uploadStatus === 'done' ? '?' : uploadStatus === 'error' ? 'Retry' : 'Upload CV'}
          </button>
        </div>
      </div>
