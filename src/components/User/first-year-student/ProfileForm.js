...
const ProfileForm = () => {
    ...
    try {
     ...
     const response = await axiosInstance.get('/api/students/profile/detailed');
      if (response.data.success && response.data.account) {
        ...
        setFieldErrors(validateProfileData(mergedData));
  ...
  const handleInputChange = (e) => {
    ...
    setFormData((prev) => {
      ...
      setFieldErrors((prevErrors) => {
        const nextErrors = {
          ...prevErrors,
          [name]: validateProfileField(name, next[name], next),
        };

        if (name === 'country') {
          nextErrors.zipCode = validateProfileField('zipCode', next.zipCode, next);
        }
  ...
  const saveProfile = async (section = null) => {
    try {
      ...
      const blockingErrors = section ? getSectionErrors(section, validationErrors) : validationErrors;
      if (hasErrors(blockingErrors)) {
        setMessage({
          type: 'error',
      ...
      if (section === 'personal') {
        updatedCompletion.personalInfo = !!(
          formData.firstName && formData.lastName && formData.birthDate && !hasErrors(getSectionErrors('personal', validationErrors))
        );
        ...
        updatedCompletion.contactDetails = !!(formData.phone && alternatePhoneValid && !hasErrors(getSectionErrors('contact', validationErrors)));
        ...
        updatedCompletion.address = !!(
          ...
          formData.zipCode &&
          formData.country &&
          !hasErrors(getSectionErrors('address', validationErrors))
        );
        ...
        updatedCompletion.geography = !!(formData.citizenshipStatus && !hasErrors(getSectionErrors('geography', validationErrors)));
      }
  ...
  const handleFinalSubmit = async () => {
    try {
      ...
      if (hasErrors(validationErrors)) {
        setMessage({
          type: 'error',
          text: 'Please fix the highlighted fields before submitting your profile.'
        });
        return;
      }
  ...
  { ...formData, profileCompletion: finalCompletion }
      ...
      setMessage({
        type: 'error',
        text: err.response?.data?.message || 'Final submission failed.'
      });
    } finally {
      setSaving(false);
    }
  ...
  const currentSectionHasErrors = hasErrors(getSectionErrors(activeSection, fieldErrors));
        ...
        {showPreview ? (
          ...
          <>
            {activeSection === 'personal' && (
              <PersonalInfoSection formData={formData} handleInputChange={handleInputChange} fieldErrors={fieldErrors} />
            )}
            ...
            {activeSection === 'address' && (
              <AddressSection formData={formData} handleInputChange={handleInputChange} fieldErrors={fieldErrors} />
            )}
            ...
            {activeSection === 'geography' && (
              <GeographySection formData={formData} handleInputChange={handleInputChange} fieldErrors={fieldErrors} />
            )}
            ...
            <div className="form-actions">
              <button
                ...
                disabled={saving || currentSectionHasErrors}
              >
